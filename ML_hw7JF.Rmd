...
en.model <- train(
  ...
  trControl = trainControl("cv", number = 10, sampling = "down"), preProc = c("center", "scale"), tuneLength = 10)
...
#Confusion Matrix
confusionMatrix(en.model) #0.5869|(accuracy) 
```
The accuracy of the elastic net model was 59%. 
...
trControl_bag = trainControl("cv", number = 10, sampling = "down")
...
bag_readmission$results #mtry: 14, Accuracy: 0.522
varImp(bag_readmission) #wbc = 100, age = 81.43, esr = 75.29, sodium 66.30
plot(varImp(bag_readmission))
confusionMatrix(bag_readmission) #Accuracy: 0.5223

```
The accuracy of the bagging model was 52%. The top three most important variables were white blood cell count (100) age at initial MI (81.43) and erythrocyte sedimentation rate (75.29). The least important variables were pulmonary adema (1.55), presence of arrythmia(0.13), and obesity (0.00). 
...
trControl_rf = trainControl("cv", number = 10, sampling = "down")
...
confusionMatrix(rf_readmission) #Accuracy (average) : 0.5516
rf_readmission$results
rf_readmission$bestTune #mtry = 6.5
rf_readmission$finalModel

varImp(rf_readmission) #age =100, wbc = 92.16, esr = 66.40, alt = 59.75
plot(varImp(rf_readmission))

varImpPlot(rf_readmission$finalModel)

```
The accuracy of the random forest model was 55%. The top three most important variables were age at initial MI (100), white blood cell count (92.2), and erythrocyte sedimentation rate (66.4). The least important variables were pulmonary adema (2), asthma (0.8), and obesity (0.00). 


## Selecting an "optimal" model and calculate final evaluation metrics in the test set.
The optimal model I selected was the elastic net model because the accuracy was the highest out of the three models I've constructed.
...
en_pred_test <- predict(en.model, test_data)

#Get evaluation metrics from test set
confusionMatrix(en_pred_test, test_data$readmission, positive = "Yes") #Accuracy 0.5933 #Sensitivity:0.59574 #Specificity:0.59307

#Create ROC Curve for Analysis
pred.prob <- predict(en.model, test_data, type = "prob")
...
abline(a = 0,b = 1)
```
The accuracy of the elastic net model on the test data was 59.33% with a sensitivity of 0.59574 and a specificity of 0.59307.
